...
void Loader::processNode( aiNode *const node, const aiScene *scene, std::vector< MeshAsset >& meshAssets ) {
    for ( std::uint32_t meshArrayIndex{}; meshArrayIndex < node->mNumMeshes; meshArrayIndex++ ) {
    ...
    for ( std::uint32_t childIndex{}; childIndex < node->mNumChildren; childIndex++ )
...
MeshAsset Loader::processMesh( aiMesh *const mesh ) {
    ...
    for ( std::uint32_t vertexID{}; vertexID < mesh->mNumVertices; vertexID++ ) {
    ...
    aiFace face{};
    for ( std::uint32_t faceID{}; faceID < mesh->mNumFaces; faceID++ ) {
        face = mesh->mFaces[ faceID ];
        for ( std::uint32_t indexID{}; indexID < face.mNumIndices; indexID++ )
